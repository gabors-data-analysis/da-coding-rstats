...
ggplot(data = vienna, aes(x = distance, y = lnprice_resid)) +
  geom_point(color = 'red', size = 2 ) + 
  geom_smooth( method="lm", colour='blue', se=F , formula = y~x) +
...
# Model:
reg_poly <- feols( lnprice ~ poly(distance, 3) + poly(rating, 3) + star3 + star35 ,
                   data = vienna , vcov = 'hetero' )

# Predictions and Errors:
# for log values
vienna$lnprice_hat_rp   <- reg_poly$fitted.values
vienna$lnprice_resid_rp <- reg_poly$residuals
# for the levels
corr_term_rp <- exp( mean( vienna$lnprice_resid_rp ) / 2 )
vienna$price_hat_rp   <- exp( vienna$lnprice_hat_rp ) * corr_term_rp
vienna$price_resid_rp <- vienna$price - vienna$price_hat_rp

# Create a residual vs distance graph -> model does not uses hotels closer to the city than 2 miles
# Furthermore if we extrapolate, we can see that the errors are going to be positive, thus it is going to 
# underprice our hotel along the distance direction. This is rather serious.
ggplot(data = vienna, aes(x = distance, y = price_resid_rp)) +
  geom_point(color = 'red', size = 2 ) + 
  geom_smooth( method="lm", colour='blue', se=F , formula = y~x) +
...
# Create a residual vs ratings graph -> model makes large errors around 4, 
#   in both directions, but mainly underprice the hotels. Not too serious.
ggplot(data = vienna, aes(x = rating, y = price_resid_rp)) +
  geom_point(color = 'red', size = 2 ) + 
  geom_smooth( method="lm", colour='blue', se=F , formula = y~x) +
  labs(x = "Ratings of the hotels",y = "Residuals")+
  scale_x_continuous(expand = c(0.01,0.01), limits=c(2, 5), breaks=seq(2, 5, by=0.5)) + 
...
nh <- tibble( distance = 0.25 , star3 = 0 , star35 = 0, rating = 4.1 )
CI_nh <- predict( reg_poly , newdata = nh , se.fit=T , interval = 'confidence' , level = 0.8 )
PI_nh <- predict( reg_poly , newdata = nh , se.fit=T , interval = 'prediction' , level = 0.8 )

nh_prices <- tibble( price   = exp( CI_nh$fit )*corr_term_rp,
                     CI_low  = exp( CI_nh$ci_low )*corr_term_rp,
                     CI_high = exp( CI_nh$ci_high )*corr_term_rp,
                     PI_low  = exp( PI_nh$ci_low )*corr_term_rp,
                     PI_high = exp( PI_nh$ci_high )*corr_term_rp )
...
hotels_cities <- europe %>% filter( accommodation_type=="Hotel" ) %>%
...
# Compare:
etable( reg_v , reg_a , reg_b , headers = c("Vienna","Amsterdam","Barcelona"))
