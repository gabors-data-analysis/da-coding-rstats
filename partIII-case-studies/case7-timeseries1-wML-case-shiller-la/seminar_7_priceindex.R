...
hprice <- read_csv('https://osf.io/n3jty/download')
...
# Create date format
hprice <- hprice %>% mutate(date = yearmonth(date))
...
hprice <- hprice %>% filter(year <= 2017)
...
# Last year of data
data_holdout_2<- hprice_all %>%
  slice((n()-11):n())

# Rest of data for work set
data_work_2 <- hprice_all %>%
  slice(1:(n()-12))
